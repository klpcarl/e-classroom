...
//
BOOL CTrayIcon::SetIcon(HICON hicon, LPCWSTR lpTip) 
{
	UINT msg;
	m_nid.uFlags = 0;
	...
	// Use the tip, if any
	if (lpTip)
		wcsncpy_s(m_nid.szTip, lpTip, sizeof(m_nid.szTip));
	if (m_nid.szTip[0])
...
LRESULT CTrayIcon::OnTrayNotification(WPARAM wID, LPARAM lEvent)
{
	...
	if (lEvent==WM_RBUTTONUP) {
		...
		::EnableMenuItem(pSubMenu->m_hMenu,nDisableID,MF_DISABLED|MF_GRAYED);//将某些键设为不可用
